...
/**
  ...
  * @param { function } drawOptions.toRpx - toRpx方法
  */
export function _drawCenterText(drawData, drawOptions) {
  const { x, y, fontSize, color, baseLine, textAlign = 'left', text, opacity = 1, textDecoration = 'none',
    width, lineNum = 1, lineHeight = 0, fontWeight = 'normal', fontStyle = 'normal', fontFamily = "sans-serif" } = drawData;
  const { ctx, toPx, toRpx } = drawOptions;
  ctx.save();
  ...
  ctx.setTextBaseline(baseLine);
  ctx.setTextAlign(textAlign);
  let textWidth = toPx(ctx.measureText(text).width);
  ctx.fillText(text, toPx(x) + textWidth/2 + (toPx(width) - textWidth) / 2, toPx(y))
  ctx.restore();
  // // textDecoration
  // if (textDecoration !== 'none') {
  //   let lineY = y;
 ...
 * @param { function } drawOptions.toRpx - toRpx方法
 */
export function drawCenterText(params, drawOptions) {
  // const { ctx, toPx, toRpx } = drawOptions;
  const {
    x,
    y, text, baseLine,
  ...
  if (Object.prototype.toString.call(text) === '[object Array]') {
    ...
    text.forEach(item => {
      preText.x += item.marginLeft || 0;
      const textWidth = _drawCenterText(Object.assign(item, {
        ...preText,
      }), drawOptions);
      preText.x += textWidth + (item.marginRight || 0); // 下一段字的 x 轴为上一段字 x + 上一段字宽度
    })
  } else {
    _drawCenterText(params, drawOptions);
  }
}
